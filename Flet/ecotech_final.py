# ==============================================================================
# Archivo: ecotech_final.py
# Alumno: Angelo
# Versión: V17 (Corrección Textos y Guardado Historial Fechas)
# ==============================================================================
    ...
    def register(db: Database, username: str, password: str):
        try:
            if not username or not password:
                return {"message": "Faltan datos", "success": False}
...
class Finance:
    ...
    def get_indicator(self, indicator: str, fecha_especifica: str = None) -> float:
        try:
            ...
            # Lógica de Fallback (Solo si NO es fecha específica)
            # Si pedimos hoy y no está (ej: fin de semana), pedimos el último disponible
            if not respuesta.get("serie") and not fecha_especifica:
                url_consulta = f"{self.base_url}/{indicator}" 
                respuesta = requests.get(url_consulta).json()

            # Si sigue sin haber datos (ej: fecha muy antigua o futuro), error.
            ...
            # Usamos 'url_consulta' que contiene exactamente lo que pedimos (incluida la fecha si la hubo)
    ...
    def guardar_historial(self, indicator, valor, fecha_dato, origen_personalizado):
        try:
            ...
            print(f"Historial guardado: {indicator} | {origen_personalizado}") # Debug en consola
